...
def text_models_batch(config, pipe, img, prompt, task_prompt, queries, max_length=512):
    input_strs = [prompt.safe_substitute(user_query=task_prompt + query) + "\n1." for query in queries]
    
    num_batches = math.ceil(len(input_strs) / config["batch_size"])
    output_texts = []
    
    for i in range(num_batches):
        batch_inputs = input_strs[i * config["batch_size"]:(i + 1) * config["batch_size"]]
        batch_results = llava_inference_simple(pipe, img, batch_inputs)
        output_texts.extend(batch_results)
...
def make_and_save_answers(config, model_config, prompt, folder_path):
    ...
    with ThreadPoolExecutor(max_workers=config["max_workers"]) as executor:
        ...
        for i in tqdm(range(0, len(batch_data), batch_size), desc="Generating answers...", total=math.ceil(len(batch_data) / batch_size)):
            ...
            futures.append(executor.submit(text_models_batch, config, model_config, img, prompt, task_prompt, batch_queries))
        
        for future in as_completed(futures):
            inf_results = future.result()
